        ...
        <div>
            ...
            <section className="relative isolate overflow-hidden" style={{ backgroundColor: theme.bg }}>
                ...
                <motion.div
                    ...
                    className="pointer-events-none absolute right-[22%] top-8 hidden text-xl sm:block sm:text-2xl"
                ...
                <motion.div
                    ...
                    className="pointer-events-none absolute right-[6%] top-1/2 hidden -translate-y-1/2 select-none text-4xl opacity-90 sm:block sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl"
                >
                    {meta.emoji}
                </motion.div>

                <div className="relative z-10 w-11/12 max-w-[1400px] mx-auto py-5 sm:py-7 md:py-9 lg:py-12">
                    <nav aria-label="Breadcrumb" className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-[11px] md:text-xs mb-2.5 sm:mb-3 md:mb-4" style={{ fontFamily: "var(--font-body)" }}>
                        <Link href="/" className="font-medium text-[#1E2B2B]/60 hover:text-[#1E2B2B] transition-colors">Home</Link>
                        <FiChevronRight className="text-[#1E2B2B]/45 text-[9px] sm:text-[10px]" />
                        <Link href="/product" className="font-medium text-[#1E2B2B]/60 hover:text-[#1E2B2B] transition-colors">Products</Link>
                        {filterType && (
                            <>
                                <FiChevronRight className="text-[#1E2B2B]/45 text-[9px] sm:text-[10px]" />
                                <span className="font-semibold text-[#1E2B2B] truncate max-w-[100px] sm:max-w-[140px] md:max-w-none">{filterValue}</span>
                            </>
                        )}
                    </nav>
                    ...
                    <motion.p
                        ...
                        className="text-[9px] sm:text-[10px] md:text-xs font-bold tracking-[0.14em] sm:tracking-[0.16em] uppercase mb-1 sm:mb-1.5"
                    ...
                    {isSearch ? (
                        <motion.h1
                            ...
                            className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold text-[#1E2B2B] leading-snug tracking-tight max-w-lg"
                        ...
                        <motion.h1
                            initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.05 }}
                            className="text-xl sm:text-2xl md:text-3xl lg:text-[2.6rem] font-semibold text-[#1E2B2B] leading-tight tracking-tight max-w-lg"
                            style={{ fontFamily: "var(--font-display)" }}
                        >
                            {filterValue}
                        </motion.h1>
                    ...
                    <motion.p
                        ...
                        className="mt-1 sm:mt-1.5 text-[11px] sm:text-xs md:text-sm font-medium text-[#1E2B2B]/70 max-w-md"
            ...
            <div className="sticky top-0 z-30 border-b border-[var(--ph-border)] bg-[var(--ph-surface)]/95 backdrop-blur-sm">
                <div className="w-11/12 mx-auto py-2.5 sm:py-3 md:py-3.5 flex items-center justify-between gap-2 sm:gap-3">
                    <div className="flex items-center gap-2 sm:gap-2.5 md:gap-3.5 min-w-0">
                        <motion.button
                            ...
                            className={`
                                relative flex h-9 w-9 sm:h-10 sm:w-10 md:h-11 md:w-11 shrink-0 items-center justify-center
                        ...
                        >
                            ...
                            {activeFilterCount > 0 && (
                                <span
                                    className="absolute -top-1 -right-1 flex h-[16px] min-w-[16px] sm:h-[18px] sm:min-w-[18px] items-center justify-center rounded-full px-1 text-[9px] sm:text-[10px] font-extrabold text-white shadow-sm"
                        ...
                        <p className="text-[11px] sm:text-xs md:text-sm font-semibold text-[var(--ph-text-soft)] truncate" style={{ fontFamily: "var(--font-body)" }}>
                    ...
                    <div ref={sortWrapRef} className="relative shrink-0">
                        <motion.button
                            ...
                            className="w-full max-w-[100px] sm:max-w-[150px] md:max-w-[180px] lg:w-[300px] flex items-center justify-between rounded-full border border-[var(--ph-border)] bg-[var(--ph-surface)] px-3 sm:px-4 lg:px-5 py-2 sm:py-2.5 text-[11px] sm:text-xs lg:text-sm font-semibold text-[var(--ph-text)] shadow-sm transition-colors duration-200 hover:border-[var(--ph-primary)]/50"
                            style={{ fontFamily: "var(--font-body)" }}
                            aria-haspopup="listbox"
                            aria-expanded={isSortOpen}
                        >
                            <span className="hidden lg:inline text-[var(--ph-text-faint)] font-medium">Sort:</span>
                            <span className="truncate">{currentSortLabel}</span>
                            <FiChevronDown className={`shrink-0 text-xs sm:text-sm text-[var(--ph-text-faint)] transition-transform duration-200 ${isSortOpen ? "rotate-180" : ""}`} />
                        </motion.button>
                        ...
                        <AnimatePresence>
                            {isSortOpen && (
                                <motion.ul
                                    ...
                                    role="listbox"
                                    className="absolute right-0 z-40 mt-2 w-full max-w-[100px] sm:max-w-[150px] md:max-w-[180px] lg:w-[300px] overflow-hidden rounded-2xl border border-[var(--ph-border)] bg-[var(--ph-surface)] py-1.5 shadow-[0_20px_50px_rgba(15,23,42,0.14)]"
                                >
                                        ...
                                        return (
                                            <li key={option.value} role="option" aria-selected={isSelected}>
                                                <button
                                                    ...
                                                    className="w-full flex items-center justify-between px-3 sm:px-4 py-2 sm:py-2.5 text-[11px] sm:text-xs lg:text-sm font-medium text-left transition-colors duration-150 hover:bg-[var(--ph-primary-soft)]"
                                                    style={{ fontFamily: "var(--font-body)" }}
                                                >
                                                    <span className={`truncate ${isSelected ? "font-bold text-[var(--ph-text)]" : "text-[var(--ph-text-soft)]"}`}>
                                                        {option.label}
                                                    </span>
                                                    {isSelected && <FiCheck className="shrink-0 text-xs sm:text-sm ml-2" style={{ color: "var(--ph-accent)" }} />}
                                                </button>
                                            </li>
                                        );
                                    })}
